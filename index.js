...
function processSum(numberList, callback) {
  let sum=0;
  numberList.forEach(function(item){sum+=item});
  return callback(sum);
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(value){
    copy.push(""+value.last_name+ ", "+value.first_name)
   } );
  return copy;
}
...
function firstNamesAllCaps(runners) {
  let newArray=runners.map(function (value){
...
function tallyUpDonations(runners) {
  return runners.reduce(function(accumulator, currentValue){return accumulator + currentValue.donation},0);
}
